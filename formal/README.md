...
| #4 DPoP htu 正規化 | `dpop::strip_query_fragment`（`&u[..u.find(['?','#'])..]`） | **本物のバグ狩り**: `&u[..end]` が多バイト境界で panic しない＋結果は接頭辞＋区切り除去。記号モデルが見ない実装固有の死角を閉じる（入力は下記の通り有界化） | `strip_query_fragment_safe` ✅ |
...
**#4 のカバレッジ境界（silent cap を作らない）**: `strip_query_fragment` は `str::find(['?','#'])` を使う。これを
**記号文字列**で走らせると、パターンマッチャの UTF-8 デコードが状態爆発する（この 8GB 環境では全 UTF-8 シンボリック化は
収束しない）。そこで入力を `"é<tail>"`（先頭は 2 バイト文字 `é` を具体値に固定、`tail` は **ASCII 全域の記号バイト**）に
有界化した。これで `find` のデコードが安くなり、記号次元を「区切りの値＝スライス位置を決める panic 面の本質」だけに絞れる。
`tail` が `?`/`#` のとき `&u[..2]`（`é` 直後 ＝ **多バイト境界**）で切れることを検証＝境界 panic 面を踏む。
**任意の先頭文字長・任意長文字列の全証明ではない**（代表的な「2 バイト文字直後の区切り」境界の証明）点を明示する。
