...
#[derive(serde::Deserialize)]
pub(super) struct CibaDemoStartReq {
    login_hint: String,
}

/// ciba-rp として backchannel 要求を発行（デモなのでクライアント認証は省略）。
pub(super) async fn ciba_demo_start(State(p): State<Arc<Provider>>, Json(req): Json<CibaDemoStartReq>) -> Response {
    let fs = match &p.firestore {
        Some(f) => f,
        None => return (StatusCode::SERVICE_UNAVAILABLE, "no firestore").into_response(),
    };
...
#[derive(serde::Deserialize)]
pub(super) struct CibaDemoPollReq {
    auth_req_id: String,
}

pub(super) async fn ciba_demo_poll(State(p): State<Arc<Provider>>, Query(q): Query<CibaDemoPollReq>) -> Response {
    let fs = match &p.firestore {
        Some(f) => f,
        None => return (StatusCode::SERVICE_UNAVAILABLE, "no firestore").into_response(),
    };
...
pub(super) async fn ciba_demo_page(State(p): State<Arc<Provider>>) -> Html<String> {
    let body = r##"<!doctype html><html lang="ja"><head><meta charset="utf-8">
