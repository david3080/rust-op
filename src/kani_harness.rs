...
/// 橋 #4（本物のバグ狩り）: htu 正規化 `strip_query_fragment` が **多バイト UTF-8 境界を含む**
/// 入力で
///   (a) panic しない（`&u[..end]` の境界スライス安全。'?'/'#' が ASCII ゆえ成立、を機械証明）
///   (b) 結果は入力の接頭辞
///   (c) 結果に '?' '#' を含まない（query/fragment を確実に落とす）
///   (d) 冪等
/// を満たすこと。記号モデル(Tamarin)が見ない実装固有の死角を実コード上で閉じる。
///
/// 2 バイト文字 `é`(U+00E9 = 0xC3 0xA9) の直後に **記号バイト 1 つ（ASCII 全域）** を置いた
/// 文字列 `"é<tail>"` で、`strip_query_fragment` が `&u[..end]` を **多バイト境界(index 2)**で
/// 切っても panic しないことを証明する。tail が `?`/`#` のとき end=2（é の途中ではなく境界）。
///
/// `str::find(['?','#'])` を**記号文字列**で走らせると、パターンマッチャの UTF-8 デコードが
/// 状態爆発する（この 8GB 環境では収束しない）。そこで **先頭文字を具体値に固定**してデコードを
/// 安くし、記号次元を「区切りの値（＝スライス位置を決める panic 面の本質）」だけに絞る。
/// **カバレッジ境界**: 任意の先頭文字長・任意長文字列の全証明ではなく、「2 バイト文字直後の区切り」
/// という代表的な境界 panic 面の証明。`find` が ASCII 区切りで常に char 境界を返すことを実コードで確認。
#[kani::proof]
#[kani::unwind(5)]
